...
const POLL_INTERVAL_MS = 5 * 60 * 1000;                // check every 5 minutes - adjust as you like
...
async function syncOnce() {
  try {
    ...
    if (!loggedSample && servers.length > 0) {
...
console.log('Starting sync bot. Checking cocogrindclub.com every', POLL_INTERVAL_MS / 60000, 'minutes.');
syncOnce(); // run once immediately on startup
